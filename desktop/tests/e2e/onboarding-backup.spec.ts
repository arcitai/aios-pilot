...
// ---------------------------------------------------------------------------
// Recovery keys are masked until the user explicitly reveals them.
// ---------------------------------------------------------------------------

test("key view masks the private key until an explicit reveal and supports copy", async ({
  page,
}) => {
  await page.context().grantPermissions(["clipboard-read", "clipboard-write"]);
  await enterMachineBackup(page);
  await expect(page.getByTestId("backup-intro-logo")).toHaveCount(0);
  const key = page.getByTestId("nsec-value");
  await expect(key).toBeVisible();
  await expect(key).not.toContainText("nsec1mock");
  await page
    .getByRole("button", { name: "Reveal private key", exact: true })
    .click();
  await expect(key).toContainText("nsec1mock");
  await page
    .getByRole("button", { name: "Hide private key", exact: true })
    .click();
  await expect(key).not.toContainText("nsec1mock");
  await page
...
test("download happy path: generated password, encrypt, native save, Next", async ({
  ...
  // The masked key card has already loaded the raw identity key;
...
test("Copy retries after an initial key read fails", async ({ page }) => {
  await page.context().grantPermissions(["clipboard-read", "clipboard-write"]);
  // The initial key read fails; explicit copy retries and succeeds.
  await installMockBridge(
    page,
  ...
  await expect(page.getByTestId("backup-copy-error")).toBeVisible();
  await expect(page.getByTestId("backup-key-well")).not.toContainText(
    "nsec1mock",
  );

  // Copy retries the read while keeping the recovered key masked.
  await page.getByTestId("backup-key-well").hover();
  await page.getByTestId("backup-copy-key").click();
  await expect(page.getByTestId("nsec-value")).toBeVisible();
