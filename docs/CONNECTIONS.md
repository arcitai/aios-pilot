...
business context. The current desktop pilot supports GitHub README imports and
Notion page search and text imports. Google remains planned.
...
asynchronous provider requests. GitHub and Notion connect, revoke, list/search,
and import actions share one UI busy lock so provider operations cannot overlap.
...
The current flow uses a Notion **internal integration token**. It does not
require Buzz to register a public OAuth application or configure an OAuth
client secret. Create an internal integration in the workspace, grant it only
the **Read content** capability, then share the pages to expose with that
integration. Notion integrations can read only content shared with them.
...
that URL as provenance only and never fetches it.

## Credential handling and desktop surface

PATs and integration tokens use Buzz's native `SecretStore` in the OS keyring.
...
does not revoke a GitHub PAT or Notion integration token at the provider. The
UI refreshes provider status after local disconnect. Revoke the token at its
provider separately when desired.
...
repository or a page and confirms import. The caller remains responsible for
storing that source through the shared business-context contract. The optional
`onConnectionStatus` callback reports a provider as connected only after its
native credential verification; checking, disconnected, and unknown states
carry `verified: false`.

## Local verification

Adapter tests use local fixture HTTP services and an injectable in-memory
credential store. They do not contact GitHub or Notion and do not use real
credentials. Live provider access remains unverified until a user configures a
token in the desktop UI.
...
and [GitHub credential security](https://docs.github.com/en/rest/authentication/keeping-your-api-credentials-secure).
